...
function set_new_tab_fields_info($mysqli, $fields_number, $xml){
	...
	if ($stmt = $mysqli->prepare($sql)) {
		...
		if (! $stmt->execute()) {
			echo "ERROR while executing table_dictionary request!";
		}
		else {
			$stmt->store_result();
		...
		echo "ERROR while preparing table_dictionary request!";
	}
	...
	for ($i = 0; $i < $values_number; $i++) {
		...
		if(!(array_key_exists($related_table_name, $ids_created))) {
			...
			//Indicate which are the parameters to put into the request
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $related_table_name . " database - Problem when trying to insert data $value in $field_name.";
				}
				else {
			...
			if ($stmt = $mysqli->prepare($sql)) {
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $related_table_name . "- Problem when trying to get key name.";
				}
				else {
		...
		//Else get the primary key and update the row
			...
			$stmt->bind_param('ss', $value, $value_id_to_update);
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $related_table_name . " database - Problem when trying to update data $value in $field_name.";
				}
				else {
					$stmt->close();
					echo"SUCCESS";
	...
	for ($i = 0; $i < $field_destinations_level2_number; $i++) {
		...
		if(array_key_exists($table_name_level2, $ids_created)) {
			...
			$stmt->bind_param('ss', $table_name_id, $value_id_to_update);
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $table_name_level2 . " database - Problem when trying to update data $table_name_id in $field_name_level2.";
				}
				else {
					$stmt->close();
					echo"SUCCESS";
		...
		//Else, the id isn't existing, there's no line into the db. Insert one.
			...
			//Indicate which are the parameters to put into the request
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $table_name_level2 . " database - Problem when trying to insert data $table_name_id (id) in $field_name_level2.";
				}
				else {
			...
			if ($stmt = $mysqli->prepare($sql)) {
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $table_name_level2 . "- Problem when trying to get key name.";
				}
				else {
	...
	if (!empty($field_destinations["level3"])) {
		...
		for ($i = 0; $i < $field_destinations_level3_number; $i++) {
			...
			$stmt->bind_param('ss', $table_name_id, $value_id_to_update);
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $table_name_level3 . " database - Problem when trying to update data $table_name_id in $field_name_level3.";
				}
				else {
					$stmt->close();
					echo"SUCCESS";
	...
	for ($i = 0; $i < $values_number; $i++) {
		...
		if ($max_level == 1) {
			...
			$stmt->bind_param('ss', $value, $thinxtra_site_id);
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $related_table_name . " database - Problem when trying to update data $value in $level1.";
				}
				else {
					$stmt->close();
				}
		...
		//Else if the level is 2 check if the field_name is set
			...
			$stmt->bind_param('s', $thinxtra_site_id);
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $table_name_level2. " database - Problem when trying to select data in $table_name_level2.";
				}
				else {
					$stmt->store_result();
			...
			if($field_value_level2 !== null) {
				...
				$stmt->bind_param('ss', $value, $field_value_level2);
					...
					if (! $stmt->execute()) {
						echo "ERROR : " . $related_table_name . " database - Problem when trying to update data $value in $level1 (where id is $field_value_level2).";
					}
					else {
						$stmt->close();
					}
			...
			if($field_value_level2 == null) {
				...
				//Indicate which are the parameters to put into the request
					...
					if (! $stmt->execute()) {
						echo "ERROR : " . $related_table_name . " database - Problem when trying to insert data $value in $level1.";
					}
					else {
				...
				//Indicate which are the parameters to put into the request
					...
					if (! $stmt->execute()) {
						echo "ERROR : " . $table_name_level2 . " database - Problem when trying to insert data $id_level1 in $field_name_level2 (id).";
					}
					else {
						$stmt->close();
					}
		...
		//Else if the level is 3, check if the field_name is set
			...
			$stmt->bind_param('s', $thinxtra_site_id);
				...
				if (! $stmt->execute()) {
					echo "ERROR : " . $table_name_level3. " database - Problem when trying to select data in $field_name_level3.";
				}
				else {
					$stmt->store_result();
			...
			if($field_value_level3 !== "") {
				...
				$stmt->bind_param('s', $field_value_level3);
					...
					if (! $stmt->execute()) {
						echo "ERROR : " . $table_name_level2. " database - Problem when trying to select data in $field_name_level2.";
					}
					else {
						$stmt->store_result();
						$stmt->bind_result($field_value_level2);
			...
			//Insert the data into level 1, get the id, insert it in level 2 and 3
				...
				$stmt->bind_param('ss', $id_level2, $thinxtra_site_id);
					...
					if (! $stmt->execute()) {
						echo "ERROR : " . $table_name_level3 . " database - Problem when trying to update data $id_level2 in $field_name_level3 (id).";
					}
					else {
						$stmt->close();
					}
