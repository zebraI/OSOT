<?php
//###################################################################################################
//FUNCTIONS - USEFUL FOR PASSWORD MODIFICATION PROCESS
//###################################################################################################
//--------------------------------------------------------------------------------
...
function check_password_size_and_user_existence($mysqli, $mail, $password) {
	...
	//------------------------------------------------------------------------------
	// Check that the user is existing
...
//--------------------------------------------------------------------------------
//Function which insert new password into database (with creation of a salt)
function set_user_password($mysqli, $mail, $password) {
	...
	$sql = "UPDATE admin_osot_user SET password = ?, salt=? WHERE mail = ?";
	
	//SQL call
	if ($stmt = $mysqli->prepare($sql)) {
		//Parameters to insert into the request
		$stmt->bind_param("sss", $password, $random_salt, $mail);
