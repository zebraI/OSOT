	...
	if ($stmt = $mysqli->prepare($sql)) {
		...
		if ($row_number == 1) {
			...
			//If OK : verify that the passwords match
				...
				//If the passwords don't match, register the connection try and its date into the database
				else {
...
//-----------------------------------------------------------------------------------------
//Function anti-bruteforce which limit the number of connections
function checkbrute($mail, $mysqli) {

	//Get the time (in seconds) et h-2
	$now = time();
	$valid_attempts = $now - (2 * 60 * 60);
	
	//Prepare the SQL
	$sql = "SELECT mail FROM admin_login_attempts WHERE mail = ? AND timestamp > '$valid_attempts'";
	...
	if ($stmt = $mysqli->prepare($sql)) {
		...
		//If more than 5 tries : "true" is returned
		if ($stmt->num_rows > 5) {
			return true;
		}
		else {
